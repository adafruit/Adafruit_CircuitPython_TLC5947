...
class TLC5947:
    ...
    class PWMOut:
        ...
        def duty_cycle(self, val: int) -> None:
            if val < 0 or val > 65535:
                raise ValueError(
                    f"PWM intensity {val} outside supported range [0;65535]"
                )
    ...
    def __init__(
        ...
        *,
        ...
        if num_drivers < 1:
            raise ValueError(
                f"Need at least one driver; {num_drivers} is not supported."
            )
        self._spi = spi
        self._latch = latch
    ...
    def _get_gs_value(self, channel: int) -> Union[int, None]:
        # pylint: disable=no-else-return
        # Disable should be removed when refactor can be tested
        if channel < 0 or channel >= _CHANNELS * self._n:
            raise ValueError(
                f"Channel {channel} not available with {self._n} board(s)."
            )
    ...
    def create_pwm_out(self, channel: int) -> Any:
    ...
    def __getitem__(self, key: int) -> int:
        ...
        """
        if key < 0:  # allow reverse addressing with negative index
